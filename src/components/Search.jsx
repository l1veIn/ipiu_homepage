    ...
    <div className="relative flex h-12 group">
      ...
      {autocompleteState.status === 'stalled' && (
        <div className="absolute inset-y-0 flex items-center right-3">
          <LoadingIcon className="w-5 h-5 animate-spin stroke-zinc-200 text-zinc-900 dark:stroke-zinc-800 dark:text-emerald-400" />
        </div>
      )}
    </div>
  )
...
export function MobileSearch() {
  ...
  return (
    <div className="contents lg:hidden">
      <button
        type="button"
        className="flex items-center justify-center w-6 h-6 transition rounded-md hover:bg-zinc-900/5 ui-not-focus-visible:outline-none lg:hidden dark:hover:bg-white/5"
        aria-label="输入查找内容"
        {...buttonProps}
      >
